...
GLuint shaderProgram ( const char * vShader , const char * fShader ) {
    GLuint program = glCreateProgram();
    checkGL(__FILE__,__LINE__,true);
    
    GLuint vertexShader = 0;
    if (initShader(&vertexShader,vShader,GL_VERTEX_SHADER)) glAttachShader(program,vertexShader);
    else fputs("Vertex Shader Initialization Error\n",stderr);
    checkGL(__FILE__,__LINE__,true);
    
    GLuint fragmentShader = 0;
    if (initShader(&fragmentShader,fShader,GL_FRAGMENT_SHADER)) glAttachShader(program,fragmentShader);
