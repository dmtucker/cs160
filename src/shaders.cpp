...
GLuint
    ...
    shaderViews[SHADERS], shaderProjections[SHADERS];
...
void updateView(glm::mat4 view)
{
    for (int i = 0; i < SHADERS; ++i) {
        ...
        glUniformMatrix4fv(shaderViews[i], 1, GL_FALSE,
                           glm::value_ptr(view));
    }
}
...
void updateProjection(glm::mat4 projection)
{
    for (int i = 0; i < SHADERS; ++i) {
        currentShader(shaders[i]);
        glUniformMatrix4fv(shaderProjections[i], 1, GL_FALSE,
                           glm::value_ptr(projection));
    }
}


void updateLight(glm::vec4 position, glm::vec4 ambient, glm::vec4 diffuse,
                 glm::vec4 specular)
{
