...
glm::vec4
    pEye = AXIS_ORIGIN, pFocus = glm::vec4(0, 0, -1, 1), vUp = AXIS_Y;

bool selectionMoveL[10],        // left
...
bool moveL = false, lookL = false,
    moveR = false, lookR = false,
    moveU = false, lookU = false,
    moveD = false, lookD = false,
    moveB = false, moveF = false, orbitCW = false, orbitCCW = false;
...
void reorient()
{
    ...
    glm::vec4 vAhead =
        glm::normalize(glm::vec4(vForward.x, 0, vForward.z, vForward.w));
    glm::vec4 vRight =
        glm::normalize(glm::vec4
                       (glm::cross(glm::vec3(vForward), glm::vec3(vUp)),
                        0));
    pFocus = vForward + pEye;

    glm::mat4 orbit = glm::mat4();
    const GLfloat velocity = rad(3.0);
    if (orbitCW xor orbitCCW)
        orbit = (orbitCCW) ?
    ...
    glm::mat4 view =
        glm::lookAt(glm::vec3(pEye), glm::vec3(pFocus), glm::vec3(vUp));
    ...
    updateLight(pSelection[0], light_ambient, light_diffuse,
                light_specular);
}
...
bool toggleLight(bool enable = not lighting)
{
    ...
    static glm::vec4 previous_ambient, previous_diffuse, previous_specular;
